...
function normalizeShift(rawShift) {
  const shift = rawShift?.toString().toLowerCase();
  if (shift === 's1' || shift === 'shift1') return 'S1';
  if (shift === 's2' || shift === 'shift2') return 'S2';
  if (shift === 's3' || shift === 'shift3') return 'S3';
  return 'N/A';
}
...
const employees = users.map((user, i) => ({
  name: user.name,
  role: user.role,
  shift: normalizeShift(user.shift)  || "N/A",
  status: user.status || "N/A",
  email: user.email || "N/A",
  manager: user.reporting_manager || "N/A",
  teamsLink: user.teams_chat_link || "#",
  image: user.profile_image || `https://i.pravatar.cc/150?img=${(i % 70) + 1}`
}));

// Now call renderCards(employees) or your display logic
...
function renderCards(direction = 'right') {
  ...
  currentEmployees.forEach(user => {
    ...
    card.innerHTML = `
      ...
      <div class='card-bar'>
        <h2>${user.name}</h2>
        <p>Type: ${user.role} | Shift: ${user.shift ?? 'N/A'}</p>
        <div class="hover-details">
          <p><strong>Email:</strong> ${user.email ?? 'N/A'}</p>
          <p><strong>Status:</strong> ${user.status ?? 'N/A'}</p>
          <p><strong>Manager:</strong> ${user.manager ?? 'N/A'}</p>
          <p><a href="${user.teamsLink ?? '#'}" target="_blank">Join Teams</a></p>
        </div>
      </div>
    `;
